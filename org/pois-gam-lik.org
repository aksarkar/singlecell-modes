...
** Likelihood surface for a simulated problem
   ...
   #+BEGIN_SRC ipython
     ...
     batch_trace_keep = np.logical_and.reduce(
       (log_mu - .5 < batch_trace_x,
        batch_trace_x < log_mu + .5,
        log_phi - .5 < batch_trace_y,
        batch_trace_y < log_phi + .5))
   #+END_SRC
   ...
   #+BEGIN_SRC ipython
     ...
     sgd_trace_keep = np.logical_and.reduce(
       (log_mu - .5 < sgd_trace_x,
        sgd_trace_x < log_mu + .5,
        log_phi - .5 < sgd_trace_y,
        sgd_trace_y < log_phi + .5))
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :async t
     n_points = 100
     log_mu_grid = np.linspace(log_mu - .5, log_mu + .5, n_points)
     log_phi_grid = np.linspace(log_phi - .5, log_phi + .5, n_points)
