...
** Weighted Negative Binomial Matrix Factorization
   ...
   = p(\mz \mid \mx, \ml, \mf)
   = \operatorname{Multinomial}(\cdot; x_{ij}, \frac{l_{i1} f_{j1}}{\sum_t l_{it} f_{jt}}, \ldots, \frac{l_{iK} f_{jK}}{\sum_t l_{it} f_{jt}}) \]
   ...
   \begin{align*}
     ...
     &\propto u_{ij}^{x_{ij} + 1/\phi_{ij} - 1} \exp\left(-u_{ij}\left(\left(\sum_k l_{ik} f_{jk}\right) + 1/\phi_{ij}\right)\right)\\
     &= \operatorname{Gamma}(x_{ij} + 1/\phi_{ij}, \left(\sum_k l_{ik} f_{jk}\right) + 1/\phi_{ij})
   \end{align*}

   The expected log joint

   \begin{multline*}
     h \triangleq \E{\ln p(\mx, \mz, \mU \mid \ml, \mf)} = \sum_{i, j, k} [\E{z_{ijk}}(\E{\ln u_{ij}} + \ln (l_{ik} f_{jk})) - l_{ik} f_{jk} \E{u_{ij}} - \ln\Gamma(x_{ij} + 1)]\\
     + \sum_{i, j} [(1 / \phi_{ij}) \ln(1 / \phi_{ij}) + (\E{u_{ij}} - 1) \ln(1 / \phi_{ij}) - \E{u_{ij}}  /  \phi_{ij} - \ln\Gamma(1 / \phi_{ij})]
   \end{multline*}
   ...
   \[ \E{u_{ij}} = \frac{x_{ij} + 1/\phi_{ij}}{\left(\sum_k l_{ik} f_{jk}\right) + 1 / \phi_{ij}} \]

   Plugging in \(\E{z_{ijk}}\), \(\E{u_{ij}}\) yields the multiplicative
   updates of a form similar to WNMF. Following the arguments for WNMF, if we
   introduce weights \(w_{ij} \in \{0, 1\}\), they will enter the M step
   updates in the numerator and denominator.
   ...
   \begin{align*}
     \frac{\partial h}{\partial \theta} &= \sum_{i, j} \Bigg[1 + \ln \theta + \frac{\E{u_{ij}} - 1}{\theta} - \E{u_{ij}} - \psi(\theta)\Bigg]\\
     \frac{\partial^2 h}{\partial \theta^2} &= \sum_{i, j} \Bigg[\frac{1}{\theta} - \frac{\E{u_{ij}} - 1}{\theta^2} - \psi_1(\theta)\Bigg]
   \end{align*}
...
** Simulated NB example
   ...
   Run the methods. In WNBMF, fix the inverse dispersion to the oracle value.
   ...
   #+BEGIN_SRC ipython :async t
     ...
     for rank in range(1, 11):
       for method in ('oracle', 'ebpm_point', 'wnmf', 'wglmpca', 'wnbmf'):
         try:
           loss = getattr(scmodes.benchmark, f'imputation_score_{method}')(x, rank=rank, frac=0.1, seed=0, inv_disp=inv_disp)
         except RuntimeError:
           # WGLMPCA fails often
           loss = np.nan
         imputation_res.append([method, rank, loss])
