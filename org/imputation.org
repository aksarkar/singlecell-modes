...
** Weighted GLM-PCA
   ...
   GLM-PCA ([[https://arxiv.org/abs/1907.02647][Townes 2019]]) uses Fisher
   scoring to fit the model assuming \(h = \log\). The weighted version of the
   problem is
...
** Incomplete data VAE
   ...
   \[ \ell \triangleq \E{\sum_{i, j} w_{ij} \ln\left(\int_0^\infty \operatorname{Poisson}(x_{ij}; x_i^+ \mu(\vz_i)_j\, u_{ij})\: dp(u_{ij})\right)} - \E{\sum_{i, j, k} w_{ij} \ln q(z_{ik} \mid x_{ij})}\]

   where expectations are taken with respect to \(q\). For easy choices of
   \(p(u_{ij})\), the integrals inside the expectation are analytic. This
   formulation also suggests against making a point-Gamma assumption on
