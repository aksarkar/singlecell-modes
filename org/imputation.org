...
** Weighted Negative Binomial Matrix Factorization
   ...
   \phi_j\). To derive an EM algorithm, first note that \(p(\mz \mid \mx, \mU,
   \ml, \mf) = p(\mz \mid \mx, \ml, \mf)\). Further,
   ...
   \begin{align*}
     ...
     &\propto u_{ij}^{x_{ij} + 1/\phi_{ij} - 1} \exp\left(-u_{ij}\left(\sum_k [l_{ik} f_{jk}] + 1/\phi_{ij}\right)\right)\\
     &= \operatorname{Gamma}(x_{ij} + 1/\phi_{ij}, \sum_k [l_{ik} f_{jk}] + 1/\phi_{ij})
   \end{align*}

   The expected log joint

   \begin{multline*}
     \E{\ln p(\mx, \mz, \mU \mid \ml, \mf)} = \sum_{i, j, k} [\E{z_{ijk}}(\E{\ln u_{ij}} + \ln (l_{ik} f_{jk})) - l_{ik} f_{jk} \E{u_{ij}} - \ln\Gamma(x_{ij} + 1)]\\
     + \sum_{i, j} [(1 / \phi_{ij}) \ln(1 / \phi_{ij}) + (\E{u_{ij}} - 1) \ln(1 / \phi_{ij}) - \E{u_{ij}}  /  \phi_{ij} - \ln\Gamma(1 / \phi_{ij})]
   \end{multline*}

   leading to M step updates
   ...
   \begin{align*}
     \E{u_{ij}} &= \frac{x_{ij} + 1/\phi_{ij}}{\sum_k [l_{ik} f_{jk}] + 1/\phi_{ij}}\\
     \E{\ln u_{ij}} &= \psi(1/\phi_{ij}) + \ln \phi_{ij}
   \end{align*}

   where \(\psi(\cdot)\) denotes the digamma function. Plugging in
   \(\E{z_{ijk}}, \E{u_{ij}}\) yields the multiplicative updates proposed by
   Gouvert et al. 2018. Following the arguments for WNMF, if we introduce
   ...
   the expected log joint with respect to \(\phi\) is possible.
...
** Simulated NB example
   ...
   Simulate some data from the model, where the signal-to-noise ratio is 1.
   ...
   #+BEGIN_SRC ipython
     ...
     f = np.random.lognormal(sigma=.5, size=(p, k))
     lam = l.dot(f.T)
     u = np.random.gamma(shape=1 / lam.var(), scale=lam.var(), size=(n, p))
   ...
   #+BEGIN_SRC ipython :async t
     ...
     for rank in range(1, 11):
       for method in ('oracle', 'ebpm_point', 'wnmf', 'wglmpca', 'wnbmf'):
         try:
           loss = getattr(scmodes.benchmark, f'imputation_score_{method}')(x, rank=rank, frac=0.1, seed=0, inv_disp=1 / lam.var())
         except RuntimeError:
           # WGLMPCA fails often
           loss = np.nan
         imputation_res.append([method, rank, loss])
   ...
   #+RESULTS:
   :RESULTS:
   0 - 6b196ec4-d70b-4223-b182-1a1344c779ec
   :END:
