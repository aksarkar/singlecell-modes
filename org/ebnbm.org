...
** Application to control data
   ...
   #+BEGIN_SRC ipython :eval never
     ...
     with mp.Pool() as pool:
       ...
       fits = pool.map(ft.partial(_fit, x=x.X.A, s=s, par=par), grid)
   ...
   #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
     sbatch --partition=broadwl -n1 -c28 --exclusive --time=24:00:00 -a 0
     #!/bin/bash
   ...
   #+BEGIN_SRC ipython :ipyfile figure/ebnbm.org/ebnbm-control.png
     ...
     plt.xscale('log')
     plt.plot(fits.keys(), [fits[k][-1] for k in fits], lw=1, c='k')
     plt.xticks([1e-2, .1, 1, 10])
     plt.xlabel(r'Measurement dispersion $\theta$')
     plt.ylabel('ELBO')
     plt.tight_layout()
   #+END_SRC
