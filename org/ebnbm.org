...
** Simulation
   ...
   #+BEGIN_SRC ipython :async t
     ...
     log_mean_hat, log_inv_disp_hat, log_inv_meas_disp_hat, alpha, beta, gamma, delta, elbo = scmodes.ebnbm.ebnbm_gamma(
       x,
       s,
     ...
     pmf[rf'NB ($\hat\theta$ = {np.exp(-log_inv_meas_disp_hat):.2g})'] = st.nbinom(n=np.exp(log_inv_meas_disp_hat), p=1 / (1 + s[0] * temp * np.exp(-log_inv_meas_disp_hat))).pmf(y.reshape(-1, 1)).mean(axis=0)
   ...
   Now fit the model initializing at the Poisson-Gamma solution, as a default.

   #+BEGIN_SRC ipython :async t
     par = np.array([scmodes.ebpm.ebpm_gamma(x[:,j], s.ravel()) for j in range(x.shape[1])])
     init = np.hstack([np.exp(par[:,1]), np.exp(par[:,1] - par[:,0]), 1])
