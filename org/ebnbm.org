...
* Introduction
  ...
  \begin{align}
    x_{ij} \mid s_i, \lambda_{ij}, u_{ij} &\sim \NB(s_i \lambda_{ij}, 1 / \theta)\\
...
** VBEM algorithm for Gamma expression model
   ...
   \begin{align}
     ...
     u_{ij} \mid \theta &\sim \Gam(\theta, \theta)\\
     \lambda_{ij} \mid a_j, b_j &\sim \Gam(a_j, b_j),
   \end{align}
   ...
   \begin{multline}
     ...
     + (a_j - 1) \ln \lambda_{ij} - b_j \lambda_{ij} + a_j \ln b_j - \ln\Gamma(a_j) + (\theta - 1) \ln u_{ij} - \theta u_{ij} + \theta\ln\theta - \ln\Gamma(\theta),
   \end{multline}
   ...
   \begin{align}
     ...
     \ln p(u_{ij} \mid x_{ij}, \lambda_{ij}, a_j, b_j) &= (x_{ij} + \theta - 1) \ln \lambda_{ij} - (s_i \lambda_{ij} + b_j) u_{ij} + \const\\
     &= \Gam(x_{ij} + \theta, s_i \lambda_{ij} + b_j).
   \end{align}
   ...
   \begin{align}
     ...
     q^*(u_{ij}) &\propto \exp((x_{ij} + \theta - 1) \ln u_{ij} - (s_i \E{\lambda_{ij}} + b_j) u_{ij})\\
     &= \Gam(x_{ij} + \theta, s_i \E{\lambda_{ij}} + \theta)\\
   ...
   \begin{multline}
     \ell = \sum_{i, j} \left[ (x_{ij} + a_j - \alpha_{ij}) \E{\ln \lambda_{ij}} - (b_j - \beta_{ij}) \E{\lambda_{ij}} + (x_{ij} + \theta - \gamma_{ij}) \E{\ln u_{ij}} - (\theta - \delta_{ij}) \E{u_{ij}} - s_i \E{\lambda_{ij}} \E{u_{ij}}\right.\\
       + \left. a_j \ln b_j + \theta\ln\theta - \alpha_{ij} \ln \beta_{ij} - \gamma_{ij} \ln \delta_{ij} - \ln\Gamma(a_j) - \ln\Gamma(\theta) + \ln\Gamma(\alpha_{ij}) + \ln\Gamma(\gamma_{ij})\right] + \const,
   \end{multline}
   ...
   \begin{align}
     \frac{\partial \ell}{\partial \theta} &= \sum_{i, j} 1 + \E{\ln u_{ij}} - \E{u_{ij}} - \psi(\theta)\\
     \frac{\partial^2 \ell}{\partial \theta^2} &= -n p \psi^{(1)}(\theta)\\
...
** Simulation
   ...
   #+BEGIN_SRC ipython
     ...
     def evaluate_enbnm_gamma(n, p, n_trials=1):
       res = dict()
       for log_theta in np.logspace(-3, 3, 10):
         for i in range(n_trials):
           x, s, log_mean, log_inv_disp, _ = simulate_nb_gamma(n, p, theta=np.exp(log_theta), seed=i)
           log_mean_hat, log_inv_disp_hat, log_theta_hat, elbo = scmodes.ebnbm.ebnbm_gamma(x, s)
           res[(log_theta, trial)] = pd.DataFrame({'log_mean': log_mean,
                                                   'log_inv_disp': log_inv_disp,
                                                   'log_mean_hat': log_mean_hat,
                                                   'log_inv_disp_hat': log_inv_disp_hat,
                                                   'log_theta_hat': log_theta_hat})
       res = (pd.concat(res)
              .reset_index()
              .rename({'level_0': 'log_theta', 'level_1': 'trial'})
              )
       return res
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :async t
     x, s, lam, u, log_mean, log_inv_disp, theta = simulate_nb_gamma(n=1000, p=1, theta=0.2, seed=1)
     log_mean_hat, log_inv_disp_hat, log_theta_hat, alpha, beta, gamma, delta, elbo = scmodes.ebnbm.ebnbm_gamma(
       x,
       s,
     ...
     pmf = dict()
     pmf['Poisson'] = st.nbinom(n=np.exp(log_inv_disp), p=1 / (1 + (s * np.exp(log_mean - log_inv_disp)))).pmf(y).mean(axis=0)

     # NB measurement => Monte Carlo integral
     n_samples = 1000
     G = st.gamma(a=np.exp(log_inv_disp), scale=np.exp(log_mean - log_inv_disp))
     ...
     Ghat = st.gamma(a=np.exp(log_inv_disp_hat), scale=np.exp(log_mean_hat - log_inv_disp_hat))
     temp = Ghat.rvs(size=(n_samples, y.shape[0], 1))
     pmf[rf'NB ($\hat\theta$ = {np.exp(log_theta_hat):.2g})'] = st.nbinom(n=np.exp(-log_theta_hat), p=1 / (1 + s[0] * temp * np.exp(log_theta_hat))).pmf(y.reshape(-1, 1)).mean(axis=0)
