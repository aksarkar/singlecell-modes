...
** Unimodal assumption
   ...
   #+BEGIN_SRC ipython :async t
     malat1_counts = x.X[:,sig.loc['MALAT1', 'gene.1']].A.ravel()
     malat1_res = scmodes.ebpm.ebpm_unimodal(malat1_counts, x.obs['size'].values.ravel())
     Fx = scmodes.benchmark.gof._ash_cdf(malat1_counts - 1, malat1_res, s=x.obs['size'].values.ravel())
