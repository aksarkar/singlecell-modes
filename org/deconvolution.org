...
** Distribution deconvolution
   ...
   #+BEGIN_SRC ipython :tangle /project2/mstephens/aksarkar/projects/singlecell-modes/code/benchmark.py
     ...
     def _score_unimix(train, test, train_size_factor, test_size_factor):
       ...
       try:
         res0 = ashr.ash_workhorse(
           ...
           # Search 2 sds around plausible latent expression values
           mode=pd.Series([lam.min(), lam.max() + 2 * np.sqrt(lam.max())]))
     ...
     def evaluate_generalization(x, pool, **kwargs):
       result = {}
...
*** Speed up ash mode estimation
    ...
    To estimate the mode for gene \(j\), we optimize:

    \[ \sum_i \sum_k \int \pi_{jk} f(x_{ij} \mid g_{jk}, \lambda_0)\ dg_{jk}(\cdot \mid \lambda_0) \]

    using
    [[https://www.rdocumentation.org/packages/stats/versions/3.5.3/topics/optimize][golden
    section search]]. We have to solve an ~ash~ subproblem for each
    query. Investigate how much loss we incur for downsampling to make the
    subproblem easier.
...
*** Run the benchmark
   ...
   #+NAME: read-ipsc
   #+BEGIN_SRC ipython :tangle /project2/mstephens/aksarkar/projects/singlecell-modes/code/benchmark.py
     def ipsc(prefix='/project2/mstephens/aksarkar/projects/singlecell-qtl/data', return_df=False, n=500, seed=0):
       ...
       keep_genes = pd.read_csv(f'{prefix}/genes-pass-filter.txt', sep='\t', index_col=0, header=None).sample(n=n, random_state=seed)
   ...
   #+BEGIN_SRC ipython :eval never
     with mp.Pool(maxtasksperchild=10) as pool:
       for data in ('ipsc',):
         x = globals()[data]()
         res = evaluate_generalization(x, pool=pool, test_size=0.1, random_state=0)
   ...
   #+BEGIN_SRC sh
     ...
     with mp.Pool(maxtasksperchild=10) as pool:
       x = ipsc(n=100)
       train, val = skms.train_test_split(x)
       res = score_unimix(train, val, pool=pool)
     EOF
   #+END_SRC

   #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
     sbatch --partition=gpu2 --gres=gpu:1 --mem=32G -n1 -c4 --time=12:00:00 --job-name=benchmark --output=benchmark.out
     #!/bin/bash
