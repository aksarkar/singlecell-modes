...
** Spike-in data
   ...
   #+BEGIN_SRC ipython
     ...
     def read_c1():
       ...
       return x.loc[:,np.logical_and((x.sum(axis=0) > 0).values, keep_samples.values.ravel())].values.T
...
*** Run the benchmark
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch -a 6,16 --partition=broadwl -n1 -c28 --exclusive --time=6:00:00 --mem=16G --job-name=deconv-generalization --out=benchmark-cpu.out
      #!/bin/bash
      source activate scmodes
      python <<EOF
    ...
    #+RESULTS:
    : Submitted batch job 60156063

    #+NAME: run-ercc-benchmark-gpu
    #+BEGIN_SRC ipython :eval never :noweb tangle :tangle /project2/mstephens/aksarkar/projects/singlecell-modes/code/temp.py
      <<imports>>
      <<ercc-data>>
      for dataset in ('c1',):
        x = data[dataset]()
...
*** Look at genes favoring more complex models
    ...
    #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/chromium2-size.png
      plt.clf()
      plt.gcf().set_size_inches(3, 3)
      plt.hist(s, color='k', bins=100)
      plt.axvline(x=1000, c='r', lw=1, ls=':')
    ...
    Show that the bimodal distribution of counts is totally explained by the
    bimodal distribution of size factors. 
    ...
    #+BEGIN_SRC ipython
      ...
      def objective(theta, x, z):
        mean = np.exp(theta[0]) + 1e-8
        inv_disp = np.exp(theta[1]) + 1e-8
        return -nb_llik(x, mean, inv_disp).sum()
      ...
      def mstep(theta, x, z):
        ...
        for j in range(n_components):
          mstep = so.minimize(objective, x0=x0[:,j], args=(x, z[:,j]), method='CG')
    ...
    #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/chromium2-npmle-examples-bimodal.png
      plt.clf()
      fig, ax = plt.subplots(4, 3)
      fig.set_size_inches(7, 5)
      for a, k in zip(ax.ravel(), query):
        n, *_ = a.hist(chromium2[s < 1000,k], bins=np.arange(chromium2[:,k].max() + 2), color='k')
        a.hist(chromium2[s >= 1000,k], bottom=n, bins=np.arange(chromium2[:,k].max() + 2), color='r')
        a.text(x=.95, y=.95,
               s=f"$\Delta l$={llik_diff.loc[k]:.1f}",
               horizontalalignment='right',
               verticalalignment='top',
    ...
    :RESULTS:
    # Out[567]:
    [[file:figure/deconvolution.org/chromium2-npmle-examples-bimodal.png]]
    :END:
...
*** Investigate genes favoring more complex models
    ...
    Look at /CD3/ gene expression.

    #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/cd3-example.png
