...
*** Naive/activated T cell mixture
   ...
   #+BEGIN_SRC ipython
     benchmark = {}
     benchmark['cd8-cd19-mix'] = pd.read_csv(f'/project2/mstephens/aksarkar/projects/singlecell-modes/data/deconv-generalization/{data}.txt.gz', sep='\t', index_col=0)
     benchmark['cyto-naive-t-mix'] = (
       pd.read_csv('/project2/mstephens/aksarkar/projects/singlecell-modes/data/deconv-generalization/cyto_naive_t_mix-gpu.txt.gz', index_col=0, sep='\t')
       .merge(pd.read_csv('/project2/mstephens/aksarkar/projects/singlecell-modes/data/deconv-generalization/cyto_naive_t_mix-unimix.txt.gz', index_col=0, sep='\t'), on='gene')
       .merge(pd.read_csv('/project2/mstephens/aksarkar/projects/singlecell-modes/data/deconv-generalization/cyto_naive_t_mix-descend.txt.gz', sep='\t'), on='gene')
       .merge(pd.read_csv('/project2/mstephens/aksarkar/projects/singlecell-modes/data/deconv-generalization/cyto_naive_t_mix-npmle.txt.gz', index_col=0, sep='\t'), on='gene')
       .merge(pd.read_csv('/project2/mstephens/aksarkar/projects/singlecell-modes/data/deconv-generalization/cyto_naive_t_mix-saturated.txt.gz', index_col=0, sep='\t'), on='gene')
       .set_index('gene'))
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/synthetic-mix-benchmark.png
     ...
     for a, k in zip(ax, sorted(benchmark.keys())):
       a.boxplot((benchmark[f'{k}']['saturated'].values.reshape(-1, 1) - benchmark[f'{k}'].values)[:,:-1],
                 widths=0.25, medianprops={'color': 'k'}, flierprops={'marker': '.', 'markersize': 4})
       a.set_xticklabels(benchmark[f'{k}'].columns[:-1], rotation=90)
       a.set_xlabel('Method')
       a.set_title(k)
     ax[0].set_ylabel('Validation set deviance')
     fig.tight_layout()
