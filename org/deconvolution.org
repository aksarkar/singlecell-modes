...
** Spike-in data
   ...
   #+BEGIN_SRC ipython
     def read_chromium(sample):
       ...
       x = scanpy.filter_genes(min_counts=1)
     ...
     def read_c1():
       ...
       return x.loc[x.sum(axis=0) > 0,keep_samples.values.ravel()].T
     ...
     data = {
       ...
       'gemcode': lambda: scanpy.read('/project2/mstephens/aksarkar/projects/singlecell-modes/data/negative-controls/zheng_gemcode_control.h5ad').X.A,
       'c1': read_c1,
     }
   #+END_SRC
...
*** Run the benchmark
    ...
    #+BEGIN_SRC ipython :eval never
      ...
      with mp.Pool() as pool:
        x = data[dataset]()
        res = scmodes.benchmark.evaluate_deconv_generalization(x, pool=pool, methods=[method])
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch -a 0-14 --partition=broadwl -n1 -c28 --exclusive --time=4:00:00 --mem=16G --job-name=deconv-generalization --out=benchmark-cpu.out
      #!/bin/bash
      source activate scmodes
      python <<EOF
    ...
    #+BEGIN_SRC ipython :eval never
      ...
      for dataset in data:
        x = data[dataset]()
        res = scmodes.benchmark.evaluate_deconv_generalization(x, pool=None, methods=['gamma', 'point_gamma'])
    ...
    *Conclusions:*
    ...
    1. ZIG does not perform better than Gamma, supporting the conclusions of
       [[https://www.biorxiv.org/content/10.1101/582064v1][Svensson 2019]]
    2. ZIEF overfits
    3. There is heterogeneity which isn't adequately described by Gamma, but is
       described equally well by unimodal and NPMLE
...
*** Look at genes favoring more complex models
    ...
    #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/chromium1-npmle-examples.png
      ...
      query = benchmark['chromium1'].loc[llik_diff[llik_diff > 0].sort_values(ascending=False).head(n=12).index].index
      plt.clf()
      fig, ax = plt.subplots(4, 3)
      fig.set_size_inches(7, 5)
      for a, k in zip(ax.ravel(), query):
        a.hist(chromium1[:,k], bins=np.arange(chromium1[:,k].max() + 1), color='k')
        a.text(x=.95, y=.95,
    ...
    #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/dropseq-npmle-examples.png
      ...
      fig.set_size_inches(7, 5)
      for a, k in zip(ax.ravel(), query):
        a.hist(dropseq[:,k], bins=np.arange(dropseq[:,k].max() + 1), color='k')
        a.text(x=.95, y=.95,
               s=f"$\Delta l$={llik_diff.loc[k]:.1f}",
               horizontalalignment='right',
               verticalalignment='top',
