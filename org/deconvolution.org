...
** Spike-in data
   ...
   *Conclusions:*
   ...
   2. There is heterogeneity which isn't adequately described by Gamma, but
      [[file:deconv-examples.org::*Spike-in%20data][appears equally well
      described by unimodal/NPMLE]]
...
** MLCB plots
   ...
   Plot the improvement in validation log likelihood against Gamma baseline.

   #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/mlcb-bench.png
     plt.clf()
     fig, ax = plt.subplots(2, 3, sharex=True, sharey=False)
     fig.set_size_inches(5, 4)
     for a, k, t in zip(ax.ravel(), benchmark, ['B cell', 'Cytotoxic T', 'iPSC', 'B cell/T cell', 'Naive/cytotoxic T', 'PBMC']):
       T = benchmark[k].sub(benchmark[k]['gamma'], axis=0)
       del T['gamma']
       a.boxplot([x[~np.isnan(x)] for x in T.values.T],
                 widths=.5,
                 medianprops={'color': 'k'},
       ...
       a.set_title(t)
     for a in ax:
       a[0].set_ylabel('Diff val log lik')  
     fig.tight_layout()
   #+END_SRC
   ...
   :RESULTS:
   # Out[60]:
   [[file:figure/deconvolution.org/mlcb-bench.png]]
   :END:

   Try to plot the benchmark results using violin plots.

   #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/mlcb-bench-violin.png
     plt.clf()
     fig, ax = plt.subplots(2, 3, sharex=True, sharey=False)
     fig.set_size_inches(5, 4)
     for a, k, t in zip(ax.ravel(), benchmark, ['B cell', 'Cytotoxic T', 'iPSC', 'B cell/T cell', 'Naive/cytotoxic T', 'PBMC']):
       T = benchmark[k].sub(benchmark[k]['gamma'], axis=0)
       del T['gamma']
       vp = a.violinplot([x[~np.isnan(x)] for x in T.values.T], widths=0.75, showextrema=True)
       for pc in vp['bodies']:
         pc.set_linewidth(0)
         pc.set_color('0.4')
       for key in ('cmins', 'cmaxes', 'cbars'):
         vp[key].set_linewidth(1)
         vp[key].set_color('0.4')

       a.axhline(y=0, ls=':', lw=1, c='k')
       a.set_xticks(np.arange(1, 5))
       a.set_xticklabels(['ZIG', 'Unimodal', 'ZIEF', 'NPMLE'], rotation=90)
       a.set_title(t)
     for a in ax:
       a[0].set_ylabel('Diff val log lik')  
   ...
   :RESULTS:
   # Out[61]:
   [[file:figure/deconvolution.org/mlcb-bench-violin.png]]
   :END:

   Plot with unimodal as the baseline.

   #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/mlcb-bench-unimodal-baseline.png
     plt.clf()
     fig, ax = plt.subplots(2, 3, sharex=True, sharey=False)
     fig.set_size_inches(5, 4)
     for a, k, t in zip(ax.ravel(), benchmark, ['B cell', 'Cytotoxic T', 'iPSC', 'B cell/T cell', 'Naive/cytotoxic T', 'PBMC']):
       T = benchmark[k].sub(benchmark[k]['unimodal'], axis=0)
       del T['unimodal']
       vp = a.violinplot([x[~np.isnan(x)] for x in T.values.T], widths=0.75, showextrema=True)
       for pc in vp['bodies']:
         pc.set_linewidth(0)
       ...
       a.axhline(y=0, ls=':', lw=1, c='k')
       a.set_xticks(np.arange(1, 5))
       a.set_xticklabels(['Gamma', 'ZIG', 'ZIEF', 'NPMLE'], rotation=90)
       a.set_title(t)
     for a in ax:
       a[0].set_ylabel('Diff val log lik')  
     fig.tight_layout()
   ...
   :RESULTS:
   # Out[62]:
   [[file:figure/deconvolution.org/mlcb-bench-unimodal-baseline.png]]
   :END:

   Plot a zoomed in version of the benchmark results.

   #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/mlcb-bench-unimodal-baseline-inset.png
     plt.clf()
     fig, ax = plt.subplots(2, 3, sharex=True, sharey=False)
     fig.set_size_inches(5, 4)
     for a, k, t in zip(ax.ravel(), benchmark, ['B cell', 'Cytotoxic T', 'iPSC', 'B cell/T cell', 'Naive/cytotoxic T', 'PBMC']):
   ...
   :RESULTS:
   # Out[63]:
   [[file:figure/deconvolution.org/mlcb-bench-unimodal-baseline-inset.png]]
   :END:

   Plot the iPSC example.
   ...
   #+BEGIN_SRC ipython :ipyfile figure/deconvolution.org/mlcb-ex.png
     k = 'ENSG00000129824'
     ...
     ax[1].legend(dummy, ['Male', 'Female'], frameon=False)
     ax[1].set_ylabel('CDF')
     ax[1].set_xlabel('Latent gene expression')
   ...
   :RESULTS:
   # Out[48]:
   [[file:figure/deconvolution.org/mlcb-ex.png]]
   :END:
