...
** Spike-in data
   ...
   [[https://www.biorxiv.org/content/10.1101/582064v1][Svensson 2019]] claims
   that there should be no heterogeneity between samples in spike-ins. However,
   there is random sampling of molecules in the spike-in protocol, which will
   introduce noise. This was previously established by
   ...
   #+BEGIN_SRC ipython
     ...
     def read_c1():
       ...
       return x.loc[:,keep_samples.values.ravel()].T
   ...
   #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
     sbatch -a 6 --partition=broadwl -n1 -c28 --exclusive --time=4:00:00 --mem=16G --job-name=deconv-generalization --out=benchmark-cpu.out
     #!/bin/bash
     source activate scmodes
     python <<EOF
