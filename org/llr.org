...
** Estimate marginal log likelihood
   ...
   #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
     sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=llik --time=60:00 -a 40
     #!/bin/bash
   ...
   #+RESULTS:
   : Submitted batch job 3376369

   Shard data sets to fit unimodal and non-parametric expression models within
   the ~midway2~ time/memory limits.
