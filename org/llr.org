...
** Estimate marginal log likelihood
   ...
   #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
     sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=llik --time=24:00:00 -a 7,22,37,52-60
     #!/bin/bash
   ...
   #+NAME: shard
   #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
     sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=llik --time=5:00:00 -a 23
     #!/bin/bash
     source activate scmodes
     python <<EOF
     <<imports>>
...
** Application to biological data sets
   ...
   #+BEGIN_EXAMPLE
     ...
     cytotoxic_t             0      0            0      0
     ipsc                    0      0            0      9
     kidney                  0    757          755    753
     liver-caudate-lobe      0      0            0      0
     pbmcs_68k               0      0            2     17
     retina                  0      0           11    120
   #+END_EXAMPLE
   :END:
   ...
   #+BEGIN_SRC ipython
     ...
     for k, g in llik[llik['dataset'].isin(non_control)].groupby('dataset'):
       query = g.pivot_table(index='gene', columns='method', values='llik')
       res[k] = (query.sub(query['npmle'], axis=0) > thresh).sum(axis=0)
     res = (pd.concat(res)
            .reset_index()
            .rename({'level_0': 'dataset'}, axis=1)
            .pivot_table(index='dataset', columns='method')
   ...
   #+BEGIN_EXAMPLE
     ...
     dataset
     b_cells                0.0    4.0          5.0       7.0
     brain                  0.0  374.0        428.0     607.0
     cytotoxic_t            0.0    0.0          0.0       0.0
     cytotoxic_t-b_cells    0.0   32.0         39.0       NaN
     cytotoxic_t-naive_t    0.0   23.0         23.0       NaN
     ipsc                   0.0    0.0          0.0       1.0
     kidney                 0.0  333.0        337.0    6991.0
     liver-caudate-lobe     0.0    2.0          5.0       0.0
     pbmcs_68k              0.0  923.0       1364.0    2330.0
     retina                 2.0  180.0        191.0     397.0
   #+END_EXAMPLE
   :END:
