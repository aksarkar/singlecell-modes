...
** Application to control data sets
   ...
   #+BEGIN_SRC ipython
     def compare(llik, baseline, thresh=10, agg=sum):
       ...
       for k, g in llik.groupby('dataset'):
         ...
         res[k] = (query.sub(query[baseline], axis=0) > thresh).agg(agg, axis=0)
       res = (pd.concat(res)
              .reset_index()
              .rename({'level_0': 'dataset'}, axis=1)
   ...
   # Out[38]:
   #+BEGIN_EXAMPLE
     method  dataset  dataset        gene        point        gamma  point_gamma  \
     0       dropseq  dropseq  ERCC-00078  -765.076024  -279.353443  -264.522869
     1       dropseq  dropseq  ERCC-00131  -723.520929  -356.755781  -341.742929
     2       gemcode  gemcode  ERCC-00002 -6108.645264 -6148.879840 -6132.663007
   ...
   #+BEGIN_SRC ipython
     ...
     for k, g in llik[llik['dataset'].isin(control)].groupby('dataset'):
       query = g.pivot_table(index='gene', columns='method', values='llik')
       temp[k] = query.loc[query['npmle'] > query['unimodal'] + thresh]
     query = (pd.concat(temp)
              .reset_index()
              .rename({'level_0': 'dataset'}, axis=1)
              [['dataset', 'gene', 'point', 'gamma', 'point_gamma', 'unimodal', 'npmle']])
   ...
   #+BEGIN_EXAMPLE
     ...
     0       chromium1  ERCC-00024  -45.460132  -45.477957   -45.478005
     1       chromium1  ERCC-00069 -140.401215 -139.854752  -139.944119
     2       chromium1  ERCC-00104  -71.913572  -71.246536   -69.867679
     3       chromium1  ERCC-00154 -272.657972 -273.072955  -273.094156
     4       chromium2  ERCC-00016  -44.980123  -44.980123   -44.995680
     5       chromium2  ERCC-00041  -45.187759  -45.204457   -45.204466
     6       chromium2  ERCC-00073 -144.039409 -137.072157  -141.523956
     7       chromium2  ERCC-00075  -15.306939  -15.306939   -15.308376
     8       chromium2  ERCC-00086  -43.580107  -43.610757   -43.611190
     9       chromium2  ERCC-00116 -179.364459 -177.229879  -174.857339
     10      chromium2  ERCC-00142  -28.006585  -28.006585   -28.011618
     11      chromium2  ERCC-00164  -55.455464  -53.541417   -48.993924
     12        dropseq  ERCC-00131 -723.520929 -356.755781  -341.742929

     method    unimodal       npmle
     0      -121.644443  -45.841260
     1      -238.635997 -139.705769
     2      -123.375871  -72.716830
     3      -331.171738 -272.658181
     4      -173.512727  -44.997467
     5      -116.825209  -45.188332
     6      -198.975064 -136.364247
     7      -186.398614  -15.307474
     8      -107.533467  -43.580519
     9      -186.885089 -174.791905
     10     -148.213405  -43.356179
     11      -80.198402  -56.292263
     12     -347.518213 -337.359459
   #+END_EXAMPLE
   :END:
...
** Application to biological data sets
   ...
   For each data set, for each expression model, report the fraction of
   genes which have improvement in marginal log likelihood above some threshold
   over the Gamma expression model.

   #+BEGIN_SRC ipython :async t
     compare(llik[llik['dataset'].isin(non_control)], baseline='gamma', agg='mean')
   #+END_SRC
   ...
   #+BEGIN_EXAMPLE
     method                  point  point_gamma  unimodal     npmle
     dataset
     b_cells              0.003740     0.009974  0.090541  0.092255
     brain                0.000000     0.025800  0.088130  0.084128
     cytotoxic_t          0.003216     0.012711  0.106432  0.107657
     cytotoxic_t-b_cells  0.003912     0.038965  0.221453  0.223860
     cytotoxic_t-naive_t  0.003522     0.029779  0.227986  0.228786
     ipsc                 0.000502     0.004319  0.465703  0.479261
     kidney               0.000000     0.013791  0.113934  0.120457
     liver-caudate-lobe   0.000000     0.001825  0.002309  0.002272
     pbmc_10k_v3          0.009717     0.042902  0.130352  0.131670
     retina               0.016323     0.034836  0.162735  0.169503
   #+END_EXAMPLE
   :END:

   For each data set, for each expression model, report the fraction of genes
   which have improvement in marginal log likelihood above some threshold over
   the unimodal expression model.

   #+BEGIN_SRC ipython
     compare(llik[llik['dataset'].isin(non_control)], baseline='unimodal', agg='mean')
   #+END_SRC
   ...
   #+BEGIN_EXAMPLE
     method                  point     gamma  point_gamma     npmle
     dataset
     b_cells              0.036933  0.039582     0.040829  0.031011
     brain                0.004343  0.010048     0.011410  0.014390
     cytotoxic_t          0.107963  0.113017     0.114089  0.105054
     cytotoxic_t-b_cells  0.028283  0.033699     0.036558  0.030239
     cytotoxic_t-naive_t  0.057637  0.063721     0.065642  0.055395
     ipsc                 0.000000  0.000000     0.000100  0.001908
     kidney               0.000000  0.002796     0.002920  0.005032
     liver-caudate-lobe   0.000000  0.035275     0.035424  0.033413
     pbmc_10k_v3          0.023551  0.028162     0.028821  0.040431
     retina               0.000000  0.005474     0.006768  0.015328
   #+END_EXAMPLE
   :END:
   ...
   jointly with the mode. For these genes, try the more expensive procedure.

   #+BEGIN_SRC ipython
     import scipy.optimize as so

     def ebpm_unimodal(x, s):
       x = pd.Series(x)
       s = pd.Series(s)

       def _loss_gridmult(gridmult, mode, x, s):
         return -ashr.ash_pois(x, s, mixcompdist='halfuniform', mode=mode, gridmult=gridmult, outputlevel='loglik', control=rpy2.robjects.ListVector({'tol.svd': 0})).rx2('loglik')[0]

       def _loss_mode(mode, x, s):
         opt = so.minimize_scalar(_loss_gridmult, bracket=[1.1, 2], bounds=[1, 2], method='bounded', args=(mode, x, s), tol=1e-2)
         assert opt.success
         return opt.fun

       opt = so.minimize_scalar(_loss_mode, bracket=[0, (x / s).max()], bounds=[0, (x / s).max()], method='bounded', args=(x, s))
       mode = opt.x
       assert opt.success
       opt = so.minimize_scalar(_loss_gridmult, bracket=[1.1, 2], bounds=[1, 2], method='bounded', args=(opt.x, x, s))
       assert opt.success
       gridmult = opt.x
