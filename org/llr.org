...
** Application to biological data sets
   ...
   Report the fraction of genes for which NPMLE did better than unimodal, but
   Gamma did not.
   ...
   #+BEGIN_SRC ipython :async t
     ...
     for k, g in llik[llik['dataset'].isin(non_control)].groupby('dataset'):
       query = g.pivot_table(index='gene', columns='method', values='llik')
       temp[k] = ((query['npmle'] > query['unimodal'] + thresh) & (query['gamma'] <= query['unimodal'] + thresh)).mean(axis=0)
     res = (pd.Series(temp)
              .reset_index()
   ...
   #+BEGIN_EXAMPLE
     ...
     0              b_cells  0.001091
     1                brain  0.004939
     2          cytotoxic_t  0.001991
     3  cytotoxic_t-b_cells  0.006620
     4  cytotoxic_t-naive_t  0.003522
     5                 ipsc  0.001908
     6               kidney  0.003479
     7   liver-caudate-lobe  0.000074
     8          pbmc_10k_v3  0.012516
     9               retina  0.014134
   #+END_EXAMPLE
   :END:
