...
** Simple case
   ...
   \(i\). Further suppose we capture \(p\) fraction of the molecules. Then, we
   have two descriptions of the data:

   \[ x_1 \sim \mathrm{Binomial}(Np, n_1 / N) \]

   \[ x_1 \sim \mathrm{Hypergeometric}(Np, N, n_1) \]

   These have equal expected value \(n_1 p\). Look at the log likelihood for some settings of these parameters.
