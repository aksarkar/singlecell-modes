...
* Introduction
  ...
  1. For spike-ins (control experiments), the data do not depart from a
     ...
     variation nor biological variation involve a point mass on zero).
...
** Data
   ...
   #+BEGIN_SRC ipython
     ...
     def _mix_10x(k1, k2, min_detect=0.05, return_y=False, p=500):
       ...
       x = x.sample(n=p, axis=1, random_state=1).reset_index(drop=True)
       if return_y:
         return x, y
       else:
         return x
     ...
     def _read_10x(k):
       return scmodes.dataset.read_10x(f'/project2/mstephens/aksarkar/projects/singlecell-ideas/data/10xgenomics/{k}/filtered_matrices_mex/hg19/',
                                       return_df=True, min_detect=0.05, p=500)
     ...
     data = {
       ...
       'ipsc': lambda: scmodes.dataset.ipsc('/project2/mstephens/aksarkar/projects/singlecell-qtl/data/', p=500, return_df=True),
...
*** Run the test
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=gpu2 --gres=gpu:1 --mem=16G --job-name=gof --time=12:00:00 -a 6-11
      #!/bin/bash
    ...
    #+RESULTS:
    : Submitted batch job 63484269

    Run the CPU methods.

    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=gof --time=24:00:00 -a 1-11
      #!/bin/bash
      source activate scmodes
      python <<EOF
      <<imports>>
    ...
    #+BEGIN_SRC ipython
      gpu_res = (pd.concat([pd.read_csv(f'/project2/mstephens/aksarkar/projects/singlecell-modes/data/gof/{k}-gpu.txt.gz',
                                        sep='\t', index_col=0) for k in data],
                           keys=data.keys())
                 .reset_index(level=0)
                 .rename({'level_0': 'dataset'}, axis='columns'))
      control = list(data.keys())[:5]
      unimodal_res = (pd.concat([pd.read_csv(f'/project2/mstephens/aksarkar/projects/singlecell-modes/data/gof/{k}-unimodal.txt.gz',
                                             sep='\t', index_col=0) for k in control],
                                keys=control)
                      .reset_index(level=0)
                      .rename({'level_0': 'dataset'}, axis='columns'))
      gof_res = pd.concat([gpu_res, unimodal_res])
    #+END_SRC
...
*** Biological data sets
    ...
    #+BEGIN_EXAMPLE
      ...
      b_cells                13    2
      cytotoxic_t             6    0
      cytotoxic_t-b_cells    38    4
      cytotoxic_t-naive_t    20    6
      ipsc                   13   17
      pbmcs_68k             111  107
    #+END_EXAMPLE
    :END:
    ...
    #+BEGIN_EXAMPLE
      ...
      method                  gamma       zig
      dataset
      b_cells              0.005906  0.009950
      cytotoxic_t          0.002493  0.000000
      cytotoxic_t-b_cells  0.016205  0.011594
      cytotoxic_t-naive_t  0.008925  0.024896
      ipsc                 0.026000  0.034000
      pbmcs_68k            0.274752  0.264851
    #+END_EXAMPLE
    :END:
