...
** Test for goodness of fit
   ...
   :END:
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=gpu2 --gres=gpu:1 --mem=16G --job-name=gof --time=1:00:00 -a 9-10
      #!/bin/bash
      ...
      for k in list(data.keys())[:6]:
        ...
        if not isinstance(x, anndata.AnnData):
          if k in sig.index:
            ...
            # genes
            t = anndata.AnnData(x.loc[:,sig.loc[k, 'gene']].values, var=sig.loc[k, 'gene'].to_frame())
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=gof --time=12:00:00 -a 0-5
      #!/bin/bash
      source activate scmodes
      python <<EOF
      <<imports>>
...
** Application to biological data sets
    ...
    #+BEGIN_EXAMPLE
      ...
      dataset
      b_cells                9.0       7.0    8.0
      cytotoxic_t            6.0       0.0    6.0
      cytotoxic_t-b_cells   34.0      23.0   18.0
      cytotoxic_t-naive_t   20.0      10.0   19.0
      ipsc                 158.0       7.0  155.0
      kidney                 NaN    2600.0    NaN
      pbmcs_68k            363.0      22.0  356.0
    #+END_EXAMPLE
    :END:
    ...
    #+BEGIN_EXAMPLE
      ...
      pbmcs_68k            0.944171  0.939394  0.945248
    #+END_EXAMPLE
    :END:
    ...
    *Remark* We previously fit point-Gamma distributions to the iPSC data
    ...
    #+BEGIN_EXAMPLE
      dataset
      b_cells                0.222222
      cytotoxic_t            0.166667
      cytotoxic_t-b_cells    0.529412
      cytotoxic_t-naive_t    0.150000
      ipsc                   0.063291
      pbmcs_68k              0.079890
      dtype: float64
    #+END_EXAMPLE
   ...
   #+BEGIN_SRC ipython :async t
     ...
     for k, g in sig.groupby(['dataset']):
       ...
       if k != 'pbmcs_68k':
         ...
         counts.append(pd.DataFrame(x[:,x.var[0].isin(query)].X.A, index=x.obs[0], columns=query))
       keys.append(k)
