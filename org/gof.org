...
** Poisson-unimodal goodness of fit
   ...
   \begin{align*}
     \Pr(x_i = x) &= \int_0^\infty \operatorname{Poisson}(x_i; x_i^+ \lambda_i) \operatorname{Uniform}(a, b) d\lambda_i\\
     &= \frac{1}{b - a} \int_a^b \operatorname{Poisson}(x_i; x_i^+ \lambda_i) d\lambda_i\\
     &= \frac{1}{x_i^+ (b - a)} \int_a^b \frac{(x_i^+)^{x_i + 1}}{\Gamma(x_i + 1)} \lambda_i^{(x_i + 1) - 1}\exp(-x_i^+ \lambda_i) d\lambda_i\\
     &= \frac{1}{x_i^+ (b - a)} \left( F_\Gamma(b; x_i + 1, x_i^+) - F_\Gamma(a, x_i + 1, x_i^+) \right)\\
...
** Real data examples
   ...
   Get examples of genes which significantly depart from the estimated
   ...
   /RPS27/ departs from Gamma and point-Gamma.
   ...
   Look at the randomized quantiles.

   #+BEGIN_SRC ipython
     F = scmodes.benchmark.gof._ash_cdf(x.X[:,0] - 1, res, x.obs['size'])
     f = scmodes.benchmark.gof._ash_pmf(x.X[:,0], fit=res, s=x.obs['size'])
   #+END_SRC
