...
** Poisson-unimodal goodness of fit
   ...
   of fit, we need the PMF and CDF of \(x\), marginalized over
   \(\hat{g}\). [[https://lsun.github.io/truncash/diagnostic_plot.html#ash:_(t)_likelihood,_uniform_mixture_prior][These
   are analytic for certain choices of likelihood and prior.]]  However, in
   this specific instance, it is not. To derive the marginal PMF, consider a
   single component:
   ...
   \begin{align*}
     ...
     &= \frac{1}{s_i (b - a)} \left( F_\Gamma(b; x_i + 1, s) - F_\Gamma(a, x_i + 1, s_i) \right)\\
     \Pr(x_i \leq x) &= \sum_{k=0}^{x} \frac{1}{s_i (b - a)} \left( F_\Gamma(b; k + 1, s) - F_\Gamma(a, k + 1, s_i) \right)\\
   \end{align*}

   where \(F_\Gamma(\cdot; \alpha, \beta)\) denotes the CDF of the Gamma
   distribution with shape \(\alpha\) and rate \(\beta\). Computing this for
   each data point, for each component gives a matrix of values \(\mathbf{F} =
   [F_{ik}]\). Then, the marginal CDF of the data is given by
   \(\mathbf{F}\mathbf{w}\).
...
** Simulation
   ...
   #+BEGIN_SRC ipython
     ...
     def evaluate(num_samples, num_trials):
       ...
       result.columns = ['logodds', 'trial', 'gamma', 'zig']
       return result
   #+END_SRC
