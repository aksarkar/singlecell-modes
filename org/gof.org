   ...
   :END:
    ...
    #+BEGIN_SRC ipython :async t
      for k in list(data.keys())[6:12]:
        x = data[k]()
        x.loc[:,sig.loc[k, 'gene']].to_csv(f'/scratch/midway2/aksarkar/modes/unimodal-data/{k}.txt.gz', sep='\t', compression='gzip')
    #+END_SRC
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=gof --time=24:00:00 -a 7-11
      #!/bin/bash
      source activate scmodes
      python <<EOF
      <<imports>>
      ...
      with mp.Pool(maxtasksperchild=20) as pool:
        ...
        # memory duplication
        x = pd.read_csv(f'/scratch/midway2/aksarkar/modes/unimodal-data/{task}.txt.gz', sep='\t', index_col=0)
...
** Application to biological data sets
    ...
    #+BEGIN_EXAMPLE
      ...
      b_cells                  9         1    8
      cytotoxic_t              6         2    6
      cytotoxic_t-b_cells     34        10   18
      cytotoxic_t-naive_t     20        11   19
      ipsc                   158         2  155
      pbmcs_68k               44        11   34
    #+END_EXAMPLE
    :END:
    ...
    #+BEGIN_EXAMPLE
      method                  gamma  unimodal       zig
      dataset
      b_cells              0.998597  0.888889  0.998753
      cytotoxic_t          0.999081  0.666667  0.999081
      cytotoxic_t-b_cells  0.994885  0.705882  0.997292
      cytotoxic_t-naive_t  0.996798  0.450000  0.996958
      ipsc                 0.984132  0.996000  0.984433
      pbmcs_68k            0.993233  0.750000  0.994771
    #+END_EXAMPLE
    :END:
