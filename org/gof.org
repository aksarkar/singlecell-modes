...
** Simulated example
   ...
   #+BEGIN_SRC ipython :async t
     ...
     Fx = np.array(ashr.cdf_ash(fit, grid).rx2('y')).ravel()
   #+END_SRC
   ...
   For comparison, fit ZINB to this example.

   #+BEGIN_SRC ipython
     fit1 = scqtl.simple.fit_zinb(x, s)
     # This calls scqtl.simple.fit_zinb
     _, zinb_cdf = scmodes.deconvolve.fit_zig(x, s)
   #+END_SRC
   ...
   Fit Poisson to this example.

   #+BEGIN_SRC ipython
     fit2 = scqtl.simple.fit_pois(x, s)
   #+END_SRC
   ...
   #+BEGIN_SRC ipython :ipyfile figure/gof.org/unimodal-gof-example.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     ...
     ax[1].plot(grid, zinb_cdf, lw=1, c=cm(0), label='ZIG')
     ax[1].axvline(x=fit1[0], c=cm(0), ls=':', lw=1)
     ax[1].plot(grid, Fx, lw=1, c=cm(1), label='Unimodal')
     ax[1].axvline(x=np.array(fit.rx2('fitted_g').rx2('a'))[0], c=cm(1), ls=':', lw=1)
     ax[1].legend(frameon=False)
     ax[1].set_xlabel('Latent gene expression $\lambda$')
     ax[1].set_ylabel('CDF')
   ...
   :RESULTS:
   # Out[53]:
   [[file:figure/gof.org/unimodal-gof-example.png]]
   :END:

   Report the log likelihood.

   #+BEGIN_SRC ipython
     pd.Series({'pointmass': fit2[-1],
                'zig': fit1[-1],
                'unimodal': np.array(fit.rx2('loglik'))[0]})
   #+END_SRC
   ...
   Report the goodness of fit of the Poisson distribution.

   #+BEGIN_SRC ipython
     scmodes.benchmark.gof._gof(x, cdf=st.poisson.cdf, pmf=st.poisson.pmf, mu=s * fit2[0])
   #+END_SRC

   #+RESULTS:
   :RESULTS:
   # Out[57]:
   : KstestResult(statistic=0.017353781706450494, pvalue=0.9240420073670237)
   :END:

   Report the goodness of fit of the ZINB distribution.

   #+BEGIN_SRC ipython
     scmodes.benchmark.gof._gof(x, cdf=scmodes.benchmark.gof._zig_cdf,
                                pmf=scmodes.benchmark.gof._zig_pmf, size=s,
                                log_mu=np.log(fit1[0]), log_phi=-np.log(fit1[1]), logodds=fit1[2])
   #+END_SRC
   ...
   :RESULTS:
   # Out[28]:
   : KstestResult(statistic=0.02170999399592824, pvalue=0.7335259999575676)
   :END:

   Report the goodness of fit of the Poisson-unimodal distribution.

   #+BEGIN_SRC ipython
     scmodes.benchmark.gof._gof_unimodal('', pd.Series(x), pd.Series(s))[1:]
   #+END_SRC

   #+RESULTS:
   :RESULTS:
   # Out[58]:
   : (0.1289853772763987, 5.779830292205654e-15)
   :END:
