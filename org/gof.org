...
** Test for goodness of fit
   ...
   :END:
    ...
    each dataset. There are several reasons why a gene might depart from the
    fitted Gamma expression model:
    ...
    Under (2), a point-Gamma model will fit better. Under (3), a unimodal model
    will not fit the data. Under (4), even a fully non-parametric model will
    not fit the data.
    ...
    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=gof --time=12:00:00 -a 22
      #!/bin/bash
      source activate scmodes
      python <<EOF
      <<imports>>
    ...
    Evaluate the log likelihood ratio between the unimodal and non-parametric
    expression models.

    #+BEGIN_SRC sh :noweb eval :dir /scratch/midway2/aksarkar/modes/
      sbatch --partition=broadwl -n1 -c28 --exclusive --job-name=gof --time=12:00:00 -a 7-15
      #!/bin/bash
      source activate scmodes
      python <<EOF
      <<imports>>
...
** Application to biological data sets
    ...
    # Out[10]:
    #+BEGIN_EXAMPLE
      method               gamma  npmle  unimodal   zig
      dataset
      b_cells                 53     10        10    45
      brain                  399     74        63   400
      cytotoxic_t             35      1         0    34
      cytotoxic_t-b_cells   1077     31        19  1070
      cytotoxic_t-naive_t    945     24        17   951
      ipsc                  2251     11         6  2244
      kidney                3487   2104      1815  2934
      liver-caudate-lobe    5365   4831      4796  4559
      pbmcs_68k              422     48        22   409
      retina                 226    125       133   202
    #+END_EXAMPLE
    :END:
    ...
    #+BEGIN_EXAMPLE
      method                  gamma     npmle  unimodal       zig
      dataset
      b_cells              0.991741  0.811321  0.811321  0.992987
      brain                0.966025  0.814536  0.842105  0.965940
      cytotoxic_t          0.994640  0.971429  1.000000  0.994793
      cytotoxic_t-b_cells  0.837972  0.971216  0.982358  0.839025
      cytotoxic_t-naive_t  0.848703  0.974603  0.982011  0.847743
      ipsc                 0.773928  0.995113  0.997335  0.774631
      kidney               0.587971  0.396616  0.479495  0.653314
      liver-caudate-lobe   0.496055  0.099534  0.106058  0.571764
      pbmcs_68k            0.935097  0.886256  0.947867  0.937096
      retina               0.977506  0.446903  0.411504  0.979894
    #+END_EXAMPLE
    :END:
    ...
    *Remark* We previously fit point-Gamma distributions to the iPSC data
    ...
    Report the fraction of genes which depart from a unimodal assumption, but not
    a fully non-parametric assumption on expression variation.
    ...
    #+BEGIN_EXAMPLE
      ...
      brain                  0.111111
      cytotoxic_t                 NaN
      cytotoxic_t-b_cells    0.684211
      cytotoxic_t-naive_t    0.235294
      ipsc                   0.333333
      kidney                 0.012121
      liver-caudate-lobe     0.049833
      pbmcs_68k              0.045455
      retina                 0.150376
      dtype: float64
    #+END_EXAMPLE
    ...
    Report the number of genes where the log likelihood difference between the
    non-parametric expression model and unimodal model exceeds \(t \in (1, 10,
    100, 1000)\), for genes which did not depart from a non-parametric
    ...
    #+BEGIN_SRC ipython
      ...
      query = (gof_res[(gof_res['dataset'].isin(non_control)) & (gof_res['method'] == 'npmle')]
               .pivot_table(index=['dataset', 'gene'], columns=['method'], values=['p'])
               .groupby(level=0)
               .apply(lambda x: x.loc[x['p']['npmle'] > .05 / x.shape[0]])
               .reset_index(level=0, drop=True)
               .reset_index())
      (llr
       .merge(query, left_on=['level_0', 'gene'], right_on=['dataset', 'gene'])
       .groupby('level_0')
       ['llr']
       .apply(lambda x: pd.Series({t: (x > t).mean() for t in (1, 10, 100, 1000)}))
       .reset_index()
    ...
    #+BEGIN_EXAMPLE
      llr
      level_1                  1         10        100       1000
      level_0
      b_cells              0.069767  0.023256  0.000000  0.000000
      brain                0.092308  0.003077  0.000000  0.000000
      cytotoxic_t          0.352941  0.000000  0.000000  0.000000
      cytotoxic_t-b_cells  0.043977  0.014340  0.004780  0.000956
      cytotoxic_t-naive_t  0.027144  0.000000  0.000000  0.000000
      ipsc                 0.241964  0.005357  0.000893  0.000000
      kidney               0.000000  0.000000  0.000000  0.000000
      liver-caudate-lobe   0.005618  0.001873  0.000000  0.000000
      pbmcs_68k            0.155080  0.029412  0.000000  0.000000
      retina               0.257426  0.108911  0.049505  0.000000
    #+END_EXAMPLE
    :END:
...
** Real data examples
   ...
   \begin{align}
     \Pr(x = k) &= E_{x_{i+} \sim \hat{p}(x_{i+})}[\Pr(x = k \mid x_{i+})]\\
     &= \frac{1}{n} \sum_i \int_0^{\infty} \operatorname{Poisson}(k; x_{i+} \lambda\)\, dg(\lambda)
   \end{align}
   ...
   One possibility is that the departure from the fitted distribution is caused
   by the assumption that \(g\) does not depend on \(x_{i+}\). First, fit a
   fully non-parametric model and test for goodness of fit.
   ...
   #+BEGIN_SRC ipython :ipyfile figure/gof.org/ENSG00000177954-fit2.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(4, 2)
     plt.hist(dat.X[:,query], bins=np.arange(dat.X[:,query].max() + 1), color='k', density=True)
   ...
   Fit two expression models, one to the 10% of samples with smallest
   \(x_{i+}\), and one to the 10% of samples with largest \(x_{i+}\), then
   compare the fitted models.

   #+BEGIN_SRC ipython :async t :ipyfile figure/gof.org/ENSG00000177954-prior-by-size.png
     cm = plt.get_cmap('Dark2')
   ...
   :RESULTS:
   # Out[30]:
   [[file:figure/gof.org/ENSG00000177954-prior-by-size.png]]
   :END:
   ...
   #+BEGIN_SRC ipython :ipyfile figure/gof.org/ENSG00000122026-fit.png
     cm = plt.get_cmap('Dark2')
     plt.clf()
     plt.gcf().set_size_inches(4, 2)
     plt.hist(dat.X[:,query], bins=np.arange(dat.X[:,query].max() + 1), color='k', density=True)
   ...
   :RESULTS:
   # Out[43]:
   [[file:figure/gof.org/ENSG00000122026-fit.png]]
   :END:
