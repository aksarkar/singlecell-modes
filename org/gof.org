...
** Simulated example
   ...
   #+BEGIN_SRC ipython :async t
     ...
     data, _ = scqtl.simulation.simulate(num_samples=1000, logodds=-5, seed=0)
     x = pd.Series(data[:,0])
     s = pd.Series(data[:,1])
     lam = x / s
     fit = ashr.ash_workhorse(
       ...
       mixsd=pd.Series(np.geomspace((.5 / s).min(), lam.max(), 25)),
   ...
   #+BEGIN_SRC ipython
     pd.Series({'zig': fit1[-1], 'unimodal': np.array(fit.rx2('loglik'))[0]})
   #+END_SRC
   ...
   :RESULTS:
   # Out[119]:
   : KstestResult(statistic=0.017599923681100874, pvalue=0.9160786776117419)
   :END:

   #+BEGIN_SRC ipython
     scmodes.benchmark.gof._gof_unimodal("", pd.Series(x), pd.Series(s))
   #+END_SRC

   #+RESULTS:
   :RESULTS:
   # Out[127]:
   : ('', 0.1068304085858266, 2.1574276423860724e-10)
   :END:
