...
** Log transform
  ...
  is on the scale of \(1 / s_{ij}\). Innstead of assuming \(x_{ijk} \sim
  F_{ijk}(\cdot)\), assume:
  ...
  #+BEGIN_SRC ipython :ipyfile figure/transforms.org/zig-log-mean-var-NA18507.png
    ...
    for a, eps, t in zip(ax, np.array([0, 0.5, 1]) / s, ['0', '1/(2s)', '1/s']):
      a.set_yscale('log')
      m = np.log(mean['NA18507'] + eps) - variance['NA18507'] / np.square(mean['NA18507'] + eps)
      v = (2 * variance['NA18507']) / np.square(mean['NA18507'] + eps) + np.square(variance['NA18507']) / (mean['NA18507'] + eps) ** 4
